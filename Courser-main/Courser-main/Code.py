...
class MaxHeap:
    ...
    def pop_max(self):
        while self.heap:
            user_score, count, username = self._heap_pop()
    ...
    def peek_max(self):
        while self.heap:
            user_score, count, username = self.heap[0]
            if username is not self.REMOVED:
                return -user_score, username
            self._heap_pop()
        return None, None
...
class CourseManager:
    ...
    def suggest_courses(self):
        ...
        paths = []
        for course in self.courses_with_credits: #n times
